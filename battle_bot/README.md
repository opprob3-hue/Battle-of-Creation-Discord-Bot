...
When `OPENAI_API_KEY` is available, matchup explanations are generated by an
AI judge with a fresh, matchup-specific logical/funny reason on every fight.
If the AI call times out or fails, the local judge uses varied matchup-specific
reasons so the tournament still finishes without AI.
...
enabled in the Discord Developer Portal. `OPENAI_API_KEY` is optional.
